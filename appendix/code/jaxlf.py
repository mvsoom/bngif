...
def _bisect_exponential_rate(
    f, tol, initial_bracket, maxiter, fail_val=jnp.nan, **kwargs
):
...
def _bisect_a(p, epsilon, tol, initial_bracket, maxiter):
    def f(a, p, epsilon):
        ...
        term1 = (
            jnp.exp(-a * p["Te"])
            * (jnp.pi / p["Tp"])
            / jnp.sin(jnp.pi * p["Te"] / p["Tp"])
        )
        term2 = a
        term3 = jnp.pi / p["Tp"] * (1 / jnp.tan(jnp.pi * p["Te"] / p["Tp"]))
        lhs = factor * (term1 + term2 - term3)

        rhs = (p["T0"] - p["Te"]) / (
            jnp.exp(epsilon * (p["T0"] - p["Te"])) - 1
        ) - 1 / epsilon

        return lhs - rhs

    return _bisect_exponential_rate(
        f, tol, initial_bracket, maxiter, fail_val=0.0, p=p, epsilon=epsilon
    )
...
def _dgf(t, p, epsilon, a, **kwargs):
    ...
    def decrease(t):
        return (
            ...
            / (epsilon * p["Ta"])
            * (
                jnp.exp(-epsilon * (t - p["Te"]))
                - jnp.exp(-epsilon * (p["T0"] - p["Te"]))
            )
        )
...
def dgf(t, p, offset=0.0, tol=1e-6, initial_bracket=None, maxiter=100):
    """Calculate the glottal flow derivative (DGF) using the LF model.

    This JAX implementation can be jitted and is differentiable with
    respect to `p`. The LF model is nonzero only for values of `t` in
    `[offset, offset + T0]`, where `T0 = p["T0"]`.
    """
    if initial_bracket is None:
        initial_bracket = _get_initial_bracket(p["T0"], tol)

    epsilon = _bisect_epsilon(
        p, tol=tol, initial_bracket=initial_bracket, maxiter=maxiter
    )
    a = _bisect_a(
        p, epsilon=epsilon, tol=tol, initial_bracket=initial_bracket, maxiter=maxiter
    )
...
def consistent_lf_params(p):
    """Check whether the implicit LF equations can be solved."""
    return ~jnp.any(jnp.isnan(dgf(0.0, p)))
...
def fant_params(s):
    ...
    if s == "female vowel":
        return {"T0": 5.0, "Te": 3.25, "Tp": 2.5, "Ta": 0.3978873577297384}
    elif s == "male vowel":
        return {
